...
case class TestFeedback(r: TravisTestResult) {
  ...
  private def latestReleasedPullRequestF: Future[PullRequest] = {
    ...
    def isMergedToMaster(pr: PullRequest): Boolean =
      pr.merged_at.isDefined && pr.base.ref == githubRepo.default_branch
    ...
    for {
      closedRecentlyUpdatedPRs <- githubRepo.pullRequests.list(ClosedPRsMostlyRecentlyUpdated).all()
      mergedPRs <- Future.traverse(closedRecentlyUpdatedPRs.filter(isMergedToMaster).take(10))(pr => githubRepo.pullRequests.get(pr.number).map(_.result))
  ...
  private def checkpoint: String = {
    ...
    lazy val masterCommit:RevCommit = gitRepo.resolve(githubRepo.default_branch).asRevCommit
